...
angular
  ...
  .config ($routeProvider) ->
    $routeProvider
      ...
      .when '/taxonomy-product',
        templateUrl: 'views/taxonomy-product.html'
        controller: 'MainCtrl'

      .when '/taxonomy-product-child',
        templateUrl: 'views/taxonomy-product-child.html'
        controller: 'MainCtrl'
      ...
      .when '/visionaries',
        templateUrl: 'views/visionaries.html'
        controller: 'MainCtrl'
      ...
      .when '/single-visionaries',
        templateUrl: 'views/single-visionaries.html'
        controller: 'MainCtrl'
  ...
  .directive "resize", ($window) ->
    getComputedStyle = (element, styleProp) ->
      if element.currentStyle
        return element.currentStyle[styleProp]
      else return $window.getComputedStyle(element, null).getPropertyValue(styleProp)  if $window.getComputedStyle
      ""
    (scope, element) ->
      applyScopeVars = ->
        ...
        # $site_width = parseInt(getComputedStyle(fluidDiv, "width"), 10)
        $site_width = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth
        scope.current_zone = "a"
        if $site_width > 1280
          scope.current_zone = "a"
        else if $site_width >= 1024
          scope.current_zone = "b"
        # 768 ipad port
        # 736 iphone+ land
        else if $site_width >= 696
          scope.current_zone = "c"
        # 414 iphone+ port
        else if $site_width >= 468
          scope.current_zone = "d"
        # 375 iphone port
        # 667 iphone land
        else if $site_width >= 0
          scope.current_zone = "e"
        else
          scope.current_zone = "b"
        return
      fluidDiv = element[0].querySelector("#fluidDiv")
  ...
  .directive "svgIcon", ->
    link = (scope, element, attrs) ->
      path = (icon) ->
        "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 32 32\">" + "<path d=\"" + icons[icon] + "\"/>" + "</svg>"
      renderSVG = ->
